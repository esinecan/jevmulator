...
that client holds one handler thread for as long as it chooses.

A socket timeout alone does not achieve the bound. `rfile.read(n)` is a buffered read that
loops over several underlying receives without returning, so a deadline checked around that
call never runs, and every trickled byte resets the per-socket timeout.
`Handler._read_bounded` therefore reads through `read1`, which returns after one underlying
receive, and sets the socket timeout to the remaining budget before each one. Both the body
read and the drain loop use it, so one absolute deadline covers the whole body however the
client paces its bytes. An incomplete body returns 408 and closes the connection.
...
- `stop` and `status` act on a process only when three proofs all hold, and **fail closed**
  ...
  command line must be readable, must invoke `-m jevmulator serve` as three adjacent
  arguments, and must pass a `--state-dir` argument whose normalized full path equals this
  checkout's state directory exactly. The command line is parsed into arguments by
  `lib/JevmulatorIdentity.ps1`, which the scriptlet dot-sources and refuses to run without.
  A substring comparison was not enough: it accepted a prefix collision such as
  `...\.jevmulator-other`, and it accepted the expected path appearing inside an unrelated
  argument. A reused process id, an unverifiable process and a daemon
